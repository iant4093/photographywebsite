...
class FrontDoorCoverageContractTests(unittest.TestCase):
    def test_all_http_handlers_verify_before_any_business_logic(self) -> None:
        ...
        for logical_id, module_name in handlers.items():
            with self.subTest(function=logical_id, module=module_name):
                ...
                body = handler.body
                if module_name in {"update_album", "add_images", "update_image", "delete_album", "delete_images"}:
    ...
    def test_http_payload_cannot_forge_internal_album_work(self):
        import add_images, update_image, update_album, delete_album, delete_images
        for module, kind in ((delete_album, "album-deletion"), (delete_images, "album-media-deletion"), (add_images, "album-upload-followup"), (update_image, "album-thumbnail-cleanup"), (update_album, "album-visibility")):
