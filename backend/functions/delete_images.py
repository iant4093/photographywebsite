...
def handler(event, context):
    ...
    try:
        ...
        with album_lease(table, album_id, context):
            ...
            mutation["UpdateExpression"] = mutation_expression(mutation["UpdateExpression"], album, mutation["ExpressionAttributeValues"])
            drive_backup_jobs.update_album(table, album, removed_keys=removed_raw_keys, **mutation)
            if mutation_protocol_enabled() and (album.get("mediaStoreVersion") == 1 or album.get("mediaStoreDirty")):
                updated_album.pop("mediaStoreVersion", None)
                updated_album["mediaStoreDirty"] = True
...
def _complete_deletion(album, pending, event, context):
    ...
    elif album.get("mediaStoreVersion") == 1:
        try:
