...
class PreviewBackfillTests(unittest.TestCase):
    def test_representative_canary_is_distinct_deterministic_and_complete(self):
        ...
        first, coverage, assignments = backfill_preview_v3.representative_canary(
            jobs, facts, source_sizes, 25_000_000
        )
        second, second_coverage, second_assignments = backfill_preview_v3.representative_canary(
            list(reversed(jobs)), facts, source_sizes, 25_000_000
        )

        self.assertEqual(first, second)
        self.assertEqual(coverage, {case: 1 for case in backfill_preview_v3.REPRESENTATIVE_CANARY_CASES})
    ...
    def test_representative_canary_fails_closed_when_a_case_is_missing(self):
        ...
        with self.assertRaisesRegex(ValueError, "private"):
            backfill_preview_v3.representative_canary([job], facts, sizes, 25_000_000)
    ...
    def test_representative_canary_uses_only_currently_pending_jobs(self):
        ...
        ready = record(
            albumId=album_ids[0],
            mediaId=backfill_preview_v3.media_id_for_key(ready_key),
            status="ready",
            previewVersion=backfill_preview_v3.PREVIEW_VERSION,
            previewKeys=backfill_preview_v3.expected_preview_keys(album_ids[0], ready_key),
        )
        jobs, _ = backfill_preview_v3.build_backfill_plan(albums, [ready])
        facts = backfill_preview_v3.selection_facts(albums, jobs)
        sizes = {
            (job["albumId"], job["rawKey"]): 30_000_000 if job["albumId"] == album_ids[5] else 1_000_000
            for job in jobs
        }

        selected, _, _ = backfill_preview_v3.representative_canary(
    ...
    def test_canary_digest_binds_full_plan_selection_assignments_and_threshold(self):
        job = {"albumId": ALBUM_ID, "rawKey": RAW_KEY, "previewVersion": backfill_preview_v3.PREVIEW_VERSION}
        base = backfill_preview_v3.representative_canary_digest(
            "a" * 64, [job], {"public": "b" * 64}, 25_000_000
        )
        self.assertNotEqual(base, backfill_preview_v3.representative_canary_digest(
    ...
    def test_representative_apply_requires_both_additional_digests(self):
        ...
        with self.assertRaisesRegex(SystemExit, "expected-full-plan-digest"):
            backfill_preview_v3.validate_apply_guards(args, "123", counts, "selected")
    ...
    def test_main_supports_a_deterministic_bounded_canary(self):
        ...
        with mock.patch.object(sys, "argv", argv), mock.patch.object(
            backfill_preview_v3, "aws_json", return_value={"Account": "123"}
        ), mock.patch.object(
            backfill_preview_v3, "stack_resource", return_value="resource"
        ), mock.patch.object(
            backfill_preview_v3, "scan_all", side_effect=[[{}], []]
        ), mock.patch.object(
            backfill_preview_v3, "build_backfill_plan", return_value=(jobs, counts)
        ), mock.patch.object(
            backfill_preview_v3,
            "validate_source_heads",
            side_effect=lambda selected, *_args: (selected, 0),
        ) as validate_heads, mock.patch("builtins.print") as output:
            self.assertEqual(backfill_preview_v3.main(), 0)
    ...
    def test_main_rejects_a_nonpositive_canary_size_before_source_reads(self):
        ...
        with mock.patch.object(sys, "argv", argv), mock.patch.object(
            ...
            backfill_preview_v3, "build_backfill_plan", return_value=([], {"plannedJobCount": 0})
        ), mock.patch.object(backfill_preview_v3, "validate_source_heads") as validate_heads:
            with self.assertRaisesRegex(SystemExit, "positive integer"):
                backfill_preview_v3.main()
    ...
    def test_plan_is_deterministic_and_skips_complete_external_metadata(self):
        ...
        jobs, counts = backfill_preview_v3.build_backfill_plan([album], [])
        self.assertEqual(jobs, [{"albumId": ALBUM_ID, "rawKey": RAW_KEY, "previewVersion": backfill_preview_v3.PREVIEW_VERSION}])
        self.assertEqual(counts["plannedJobCount"], 1)
        self.assertEqual(backfill_preview_v3.plan_digest(jobs), backfill_preview_v3.plan_digest(list(jobs)))
        ...
        jobs, counts = backfill_preview_v3.build_backfill_plan([album], [metadata])
    ...
    def test_exact_previous_ready_contract_is_planned_as_an_additive_upgrade(self):
        ...
        metadata = record(
            albumId=ALBUM_ID,
            mediaId=backfill_preview_v3.media_id_for_key(RAW_KEY),
            status="ready",
            previewVersion=backfill_preview_v3.PREVIOUS_PREVIEW_VERSION,
            previewKeys=backfill_preview_v3.previous_preview_keys(ALBUM_ID, RAW_KEY),
        )
    ...
    def test_apply_requires_every_exact_guard(self):
        ...
        with self.assertRaises(SystemExit):
            backfill_preview_v3.validate_apply_guards(args, "123", counts, "digest")
        args.confirm = "backfill-preview-v3"
        backfill_preview_v3.validate_apply_guards(args, "123", counts, "digest")
    ...
    def test_dispatch_batches_at_ten_and_uses_deterministic_bodies(self):
        ...
        with mock.patch.object(backfill_preview_v3, "aws_json", side_effect=send):
            dispatched = backfill_preview_v3.dispatch_jobs(jobs, "https://queue.example", "profile", "region")
    ...
    def test_dispatch_stops_on_partial_batch_failure(self):
        ...
        with mock.patch.object(backfill_preview_v3, "aws_json", return_value=response) as send:
            with self.assertRaisesRegex(RuntimeError, "InternalError"):
                backfill_preview_v3.dispatch_jobs(jobs, "https://queue.example", None, "region")
        send.assert_called_once()

    def test_dispatch_zero_jobs_does_not_call_sqs(self):
        with mock.patch.object(backfill_preview_v3, "aws_json") as send:
            self.assertEqual(backfill_preview_v3.dispatch_jobs([], "https://queue.example", None, None), 0)
    ...
    def test_dispatch_rejects_duplicate_jobs_before_calling_sqs(self):
        ...
        with mock.patch.object(backfill_preview_v3, "aws_json") as send:
            with self.assertRaisesRegex(ValueError, "duplicate"):
                backfill_preview_v3.dispatch_jobs([job, dict(job)], "https://queue.example", None, None)
        send.assert_not_called()
