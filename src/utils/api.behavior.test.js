...
describe('public API client behavior', () => {
  ...
  it('calls every mutation endpoint with encoded identifiers, authorization, JSON, and null 204 handling', async () => {
    ...
    await api.requestAlbumPrintSession('a/b', 'media', 'token', { signal })
    await api.requestSharedPrintSession('share/code', 'media', { signal })
    await api.redeemPrintSession('print-token', { signal })
    ...
    await api.deleteUser('token', 'user+one@example.com', { signal, userId: 'selected-subject' })
    await api.editUser('token', 'user+one@example.com', { enabled: true }, { signal })
    expect(fetchMock).toHaveBeenCalledTimes(21)
    expect(fetchMock.mock.calls.some(([url, options]) => url.endsWith('/users/user%2Bone%40example.com')
      && options.method === 'DELETE' && options.body === JSON.stringify({ userId: 'selected-subject' }))).toBe(true)
