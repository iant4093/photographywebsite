...
describe('mutation continuation boundary', () => {
    afterEach(() => { clearApiCache({ sessionChanged: true }); vi.useRealTimers(); vi.unstubAllGlobals() })
    it('automatically completes explicit 409 contention and 202 pending responses', async () => {
        const { updateAlbum } = await import('./api')
        await import('./albumMutation')
        vi.useFakeTimers()
        const request = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ code: 'media_busy', error: 'Busy' }), { status: 409, headers: { 'Content-Type': 'application/json' } }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ pending: true }), { status: 202 }))
    ...
    it('never replays a pending save after the signed-in session changes', async () => {
        const { updateAlbum } = await import('./api')
        await import('./albumMutation')
        vi.useFakeTimers()
        const request = vi.fn().mockResolvedValue(new Response(JSON.stringify({ pending: true }), { status: 202 }))
        vi.stubGlobal('fetch', request)
        const result = updateAlbum('token', 'album', { title: 'Update' })
