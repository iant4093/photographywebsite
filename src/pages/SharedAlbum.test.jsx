...
describe('SharedAlbum access and gallery', () => {
  ...
  it('loads a bookmarked share, navigates the photo lightbox, refreshes, and downloads', async () => {
    ...
    expect(screen.getByRole('img', { name: /^Photograph \d/ })).toHaveAttribute('src', 'https://x.test/p1-full')
    ...
    const overlay = screen.getByRole('img', { name: /^Photograph \d/ }).closest('.fixed')
    fireEvent.click(overlay.querySelector('button.absolute.right-4'))
    expect(screen.getByText('2 / 2')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('img', { name: /^Photograph \d/ }).closest('.fixed').querySelector('button.absolute.left-4'))
    ...
    expect(screen.queryByRole('img', { name: /^Photograph \d/ })).toBeNull()
  ...
  it('checks one original with the share grant while retaining verification and selection', async () => {
    ...
    expect(screen.getByText('2 / 2')).toBeInTheDocument()
    expect(screen.getByAltText(/^Photograph \d/)).toHaveAttribute('src', 'https://x.test/p2-full')
