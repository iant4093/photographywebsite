...
describe('Photo Editor page', () => {
    ...
    it('does not let delayed close cleanup overwrite a newly opened photo', async () => {
        let finishClose
        mocks.clearEditorSession.mockImplementationOnce(() => new Promise(resolve => { finishClose = resolve }))
        mocks.saveEditorSource.mockResolvedValueOnce('source-a').mockResolvedValueOnce('source-b')
        const user = userEvent.setup()
        const { container } = render(<Editor />)
        await user.upload(container.querySelector('input[type="file"]'), new File(['jpeg'], 'first.jpg', { type: 'image/jpeg' }))
        ...
        expect(screen.queryByText('No saved session')).not.toBeInTheDocument()
    })
