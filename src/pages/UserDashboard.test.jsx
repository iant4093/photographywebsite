...
describe('UserDashboard', () => {
  ...
  it.each([
    ...
    expect(screen.getByRole('img', { name: /^Photograph \d/ })).toHaveAttribute('src', 'https://x.test/new-two')
  ...
  it('keeps the selected photo open when the current album refresh completes', async () => {
    ...
    fireEvent.error(screen.getByRole('img', { name: /^Photograph \d/ }))
    await waitFor(() => expect(api.fetchAlbum).toHaveBeenCalledTimes(2))
    ...
    expect(screen.getByRole('img', { name: /^Photograph \d/ })).toHaveAttribute('src', 'https://x.test/two-full?fresh=1')
    expect(screen.getByText('2 / 2')).toBeInTheDocument()
  })
